...
router.post('/favorites/toggle', (req, res) => {
    ...
    let params = [idUser];
    if (idItem) {
        checkQuery += "idItem = ?"; params.push(idItem);
    } else {
        checkQuery += "idTypeService = ?"; params.push(idTypeService);
    }
...
router.get('/favorites/:userId', (req, res) => {
    const query = `SELECT f.idFavorite, 'article' AS type, i.idItem AS id, i.name, i.price, i.image, i.onSale
