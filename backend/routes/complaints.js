...
router.get('/complaint/:id/messages', (req, res) => {
    const query = `SELECT cm.*, u.firstName, u.lastName, u.role FROM ComplaintMessage cm JOIN User u ON cm.idUser = u.idUser WHERE cm.idComplaint = ? ORDER BY cm.sendDate ASC`;
    connection.query(query, [req.params.id], (error, results) => {
        if (error) {
            return res.status(500).json({ error: 'Erreur serveur' });
        }
