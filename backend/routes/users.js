    ...
    connection.query('SELECT * FROM User WHERE email = ?', [email], (error, results) => {
        ...
        if (results.length > 0) {
            bcrypt.compare(password, results[0].password, (compareError, match) => {
                if (compareError) {
                    return res.status(500).json({ error: 'Erreur comparaison' });
                }
...
router.get('/user', (req, res) => {
    connection.query("SELECT idUser, firstName, lastName, email, phoneNumber, role FROM User ORDER BY idUser DESC", (error, results) => {
        if (error) {
            return res.status(500).json({ error: 'Erreur récupération comptes' });
        }
...
router.get('/user/:id/bookings', (req, res) => {
    const query = `SELECT s.idService, s.date, s.heure, s.duree, ts.name, ts.image FROM Participation p JOIN Service s ON p.idService = s.idService JOIN TypeService ts ON s.idTypeService = ts.idTypeService WHERE p.idUser = ? ORDER BY s.date DESC, s.heure DESC`;
...
router.get('/user/:userId/hasBought/:articleId', (req, res) => {
    ...
    connection.query(query, [req.params.userId, req.params.articleId], (error, results) => {
        if (error) {
            return res.status(500).json({ error: 'Erreur vérification achat' });
        }
